...
local function secpreciouschange(self, preciouschange)
    self.inst.secpreciouschange:set(preciouschange)
end
...
function seplayerstatus:numget()
    ...
    local check = false
    for i = 1, #self.slist do if num == self.slist[i] then check = true end end
    if check == true then
        return self:numget()
    else
        return num
...
function seplayerstatus:DoDeltaCoin(amount)
    if amount < 0 then
        ...
        self.inst.components.talker:Say(STRINGS.SIMPLEECONOMY[9] .. amount ..
                                            STRINGS.SIMPLEECONOMY[18])
    end
    ...
    if secoin ~= nil then
        ...
        for i = 1, maxtime do
            self.inst:DoTaskInTime(FRAMES * i, function()
                ...
                y = y1 - y0
                z = z1 - z0
                secoin.Transform:SetPosition(x / (maxtime - i) + x0,
                                             y / (maxtime - i) + y0,
                                             z / (maxtime - i) + z0)
                if i == maxtime then
...
function seplayerstatus:Init(inst)
    ...
    inst:ListenForEvent("cycleschanged", function()
        print("[Player Secoins]: ", inst.userid .. "@" .. inst.name .. "@" ..
                  inst.prefab .. "@" .. self.coin)
        self.day = self.day + 1
        if self.day >= 3 then
            self.day = 0
            self:preciousbuild()
