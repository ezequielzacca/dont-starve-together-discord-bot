...
local function goldstafffn(staff, target, pos)
    ...
    for k, v in pairs(ents) do
        if v.components.inventoryitem and v.components.inventoryitem.owner ==
            nil and v.prefab ~= "secoin" and v.prefab ~= "chester_eyebone" and
            v.prefab ~= "glommerflower" and v.prefab ~= "hutch_fishbowl" and
            staff.components.finiteuses.current >= 1 then
            local price = 0
            local stacksize = 1
            ...
            if v.components.armor and v.components.armor.maxcondition > 0 and
                v.prefab ~= "vipcard" then
            ...
            staff.components.inventoryitem.owner.SoundEmitter:PlaySound(
                "dontstarve/common/staff_dissassemble")
    ...
    if talkprice > 0 then
        ...
        caster:DoTaskInTime(pass * FRAMES * 3 + .4, function()
            caster.components.talker:Say(
                STRINGS.SIMPLEECONOMY[10] .. talkprice ..
                    STRINGS.SIMPLEECONOMY[19])
...
local function onattack(self, attacker, target, projectile)
    local maxhp = 25
    local chance = 25
    if TUNING.allowgoldstaff == false then chance = 100 end
    if attacker then
    ...
    if target and target:IsValid() and target.components.health and
        target.components.health.currenthealth <= maxhp and math.random(0, 100) <=
        chance and self.inst.components.finiteuses.current >= 1 and
        target.components.lootdropper and
        (target.components.freezable or target:HasTag("monster")) then
        local amount = target.components.health.maxhealth
        SpawnPrefab("explode_small_slurtle").Transform:SetPosition(
            target:GetPosition():Get())
        ...
        item.components.secoin.amount = math.random(math.ceil(amount / 16),
                                                    math.ceil(amount / 8))
...
local function fn()
    ...
    inst.components.inventoryitem.atlasname =
        "images/inventoryimages/goldstaff.xml"
