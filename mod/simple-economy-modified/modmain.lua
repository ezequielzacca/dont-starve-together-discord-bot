...
PrefabFiles = {"secoin", "stealer", "goldstaff", "vipcard", "luckamulet"}
...
AddPrefabPostInit("forest",
                  function() GLOBAL.TheWorld:AddComponent("seworldstatus") end)
AddPrefabPostInit("cave",
                  function() GLOBAL.TheWorld:AddComponent("seworldstatus") end)
...
AddModRPCHandler("SimpleEconomy", "buy", function(player, iname, iprice, more)
    ...
    if player.components.health.currenthealth > 0 and
        not player:HasTag("playerghost") then
        local discount = player.components.seplayerstatus.discount
        if player.components.seplayerstatus.coin >=
            math.ceil(iprice * discount * amount) then
            for i = 1, amount do
                local item = GLOBAL.SpawnPrefab(iiname)
                player.components.inventory:GiveItem(item, nil,
                                                     player:GetPosition())
            end
        ...
        if player.components.seplayerstatus.coin >= math.ceil(iprice * discount) then
            ...
            for i = 1, amount do
                ...
                loots.Physics:SetVel(2 * math.cos(angle), 10,
                                     2 * math.sin(angle))
            end
            player.components.seplayerstatus:DoDeltaCoin(-iprice * amount)
        end
...
local function Adduiseconomy(self)
    ...
    self.OnUpdate = function(self, dt)
        ...
        if curscreensize[1] ~= screensize[1] or curscreensize[2] ~=
            screensize[2] then
            ScaleUI(self)
    ...
    AddPrefabPostInitAny(function(inst)
        inst:ListenForEvent("death", function()
            ...
            if not inst:HasTag("wall") and inst:IsValid() and
                ...
                local bounty = math.floor(
                                   (inst.components.health.maxhealth / 12) *
                ...
                if bounty > 0 then
                    ...
                    for k, player in pairs(
                                         GLOBAL.FindPlayersInRange(
                                             ...
                                             true)) do
                        local item =
                            inst.components.lootdropper:SpawnLootPrefab("secoin")
